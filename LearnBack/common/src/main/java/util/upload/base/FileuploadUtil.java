...
import util.properties.PropertiesUtils;
import util.redis.UUIDUtil;
...
public class FileuploadUtil {
    ...
    private static String getDestPath(String childFile, String extName) {
        //规则：  子目录/UUID 随机数.后缀名
        String sb = "/" + UUIDUtil.getUUID() + "." + extName;
        return sb;
    }
     ...
     * 方法描述：上传文件
     */
    public static FileResult saveFile(MultipartFile multipartFile) throws IOException {
        return saveFile(multipartFile, null, null, false);
    }

    /**
     * 方法描述： 上传图片
     */
    public static FileResult saveImage(MultipartFile multipartFile) throws IOException {
        return saveFile(multipartFile, null, null, true);
    }
